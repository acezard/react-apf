const list = [
  {
    "id": "331",
    "score": 1,
    "fields": {
      "search_api_id": "331",
      "search_api_relevance": 1,
      "changed": 1478690553,
      "created": 1477405097,
      "field_address:city": [
        "Bourg la Reine"
      ],
      "field_address:country": [
        "fr"
      ],
      "field_address:country_name": [
        "France"
      ],
      "field_address:latitude": 48.77961,
      "field_address:longitude": 2.313411,
      "field_address:name": [
        "APF - SERVICE SP\u00c9CIALIS\u00c9 POUR UNE VIE AUTONOME \u00c0 DOMICILE"
      ],
      "field_address:postal_code": [
        "94210"
      ],
      "field_address:street": [
        "70 rue des rosiers"
      ],
      "field_structure_category:field_code": [
        "ADULTE"
      ],
      "field_structure_category:name": [
        "Structures pour adultes"
      ],
      "field_structure_category:tid": 65,
      "nid": 331,
      "search_api_language": "fr",
      "search_api_url": "http:\/\/localhost:8000\/content\/apf-service-sp%C3%A9cialis%C3%A9-pour-une-vie-autonome-%C3%A0-domicile",
      "status": 1,
      "title": [
        "APF - SERVICE SP\u00c9CIALIS\u00c9 POUR UNE VIE AUTONOME \u00c0 DOMICILE"
      ],
      "type": "structure",
      "url": "http:\/\/localhost:8000\/content\/apf-service-sp%C3%A9cialis%C3%A9-pour-une-vie-autonome-%C3%A0-domicile"
    }
  },
  {
    "id": "350",
    "score": 1,
    "fields": {
      "search_api_id": "350",
      "search_api_relevance": 1,
      "changed": 1478690512,
      "created": 1478686952,
      "field_address:city": [
        "Paris"
      ],
      "field_address:country": [
        "fr"
      ],
      "field_address:country_name": [
        "France"
      ],
      "field_address:latitude": 48.86409,
      "field_address:longitude": 2.38215,
      "field_address:name": [
        "Ecedi Paris"
      ],
      "field_address:postal_code": [
        "75010"
      ],
      "field_address:street": [
        "91 av de la R\u00e9publique"
      ],
      "field_structure_category:field_code": [
        "EMPLOI"
      ],
      "field_structure_category:name": [
        "Structure emploi"
      ],
      "field_structure_category:tid": 67,
      "nid": 350,
      "search_api_language": "fr",
      "search_api_url": "http:\/\/localhost:8000\/content\/ecedi",
      "status": 1,
      "title": [
        "Ecedi"
      ],
      "type": "structure",
      "url": "http:\/\/localhost:8000\/content\/ecedi"
    }
  },
  {
    "id": "351",
    "score": 1,
    "fields": {
      "search_api_id": "351",
      "search_api_relevance": 1,
      "changed": 1478690407,
      "created": 1478687029,
      "field_address:city": [
        "Grenoble"
      ],
      "field_address:country": [
        "fr"
      ],
      "field_address:country_name": [
        "France"
      ],
      "field_address:latitude": 45.186337,
      "field_address:longitude": 5.724345,
      "field_address:name": [
        "Ecedi Grenoble"
      ],
      "field_address:postal_code": [
        "38000"
      ],
      "field_address:street": [
        "30, boulevard Gambetta"
      ],
      "field_structure_category:field_code": [
        "EMPLOI"
      ],
      "field_structure_category:name": [
        "Structure emploi"
      ],
      "field_structure_category:tid": 67,
      "nid": 351,
      "search_api_language": "fr",
      "search_api_url": "http:\/\/localhost:8000\/content\/ecedi-grenoble",
      "status": 1,
      "title": [
        "Ecedi Grenoble"
      ],
      "type": "structure",
      "url": "http:\/\/localhost:8000\/content\/ecedi-grenoble"
    }
  }
]

export default function (state = list, action) {
  return state
}
